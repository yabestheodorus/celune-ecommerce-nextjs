...
import { X, Pencil, Sparkles, ImageIcon, Upload, Link2, Trash2 } from "lucide-react";
...
export default function EditProductModal({ product, categories }: { product: Product; categories: Categories }) {
  ...
  const [priceNumber, setPriceNumber] = useState(String(product.priceNumber));
  const [description, setDescription] = useState(product.description ?? "");
  const [rating, setRating] = useState(product.rating != null ? String(product.rating) : "");
  const [reviews, setReviews] = useState(product.reviews != null ? String(product.reviews) : "");
  ...
  const handleOpen = () => {
    // Reset to latest product values each time modal opens
    setName(product.name);
    setType(product.type);
    setPriceNumber(String(product.priceNumber));
    setDescription(product.description ?? "");
    setRating(product.rating != null ? String(product.rating) : "");
    setReviews(product.reviews != null ? String(product.reviews) : "");
  ...
  return (
    <>
      {/* Trigger — inline edit icon button */}
      <button
        onClick={handleOpen}
        className="p-2.5 hover:bg-white hover:text-primary-terracota rounded-xl text-[#1a1c1a]/30 transition-all duration-500 shadow-sm"
      >
        <Pencil size={16} strokeWidth={1.5} />
      </button>

      {/* Backdrop — portalled to body to escape CSS stacking contexts (backdrop-filter, overflow) */}
      {open && createPortal(
        <div
        ...
        >
          ...
          <div className="relative w-full max-w-5xl max-h-[92vh] bg-surface rounded-[40px] overflow-hidden shadow-2xl flex animate-in fade-in zoom-in-95 duration-300">
            ...
            {/* Left — Live Preview */}
            <div className="hidden lg:flex w-85 shrink-0 flex-col bg-[#EDE8E1] p-10 gap-8">
              <div>
                <p className="font-manrope text-[10px] uppercase tracking-[0.3em] font-bold text-primary-terracota">Editing</p>
              ...
              <div className="flex items-start justify-between px-10 pt-10 pb-6 border-b border-[#1a1c1a]/5">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Pencil size={13} className="text-primary-terracota" strokeWidth={1.5} />
                ...
                {step === 0 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    <Field label="Formulation Name" hint="Give it a distinctive, editorial name">
                      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Radiance Nectar Serum" className={inputCls} />
                    </Field>
                    <Field label="URL Slug" hint="Override the automated SEO-friendly URL if needed">
                      <input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="radiance-nectar-serum" className={inputCls} />
                    </Field>
                ...
                {step === 1 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    <Field label="Price (IDR)" hint="Enter the amount in Indonesian Rupiah">
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={priceNumber} onChange={(e) => setPriceNumber(e.target.value)}
                          placeholder="490000"
                          className={`${inputCls} pl-16`}
                        />
                        <span className="absolute right-5 top-1/2 -translate-y-1/2 font-manrope text-[12px] font-semibold text-primary-terracota">{formattedPrice}</span>
                      </div>
                    </Field>
                    ...
                    <Field label="Rating" hint="Score from 0.0 to 5.0">
                      <input value={rating} onChange={(e) => setRating(e.target.value)} type="number" min={0} max={5} step={0.1} placeholder="4.9" className={inputCls} />
                    </Field>
                  </div>
                )}
                ...
                {step === 2 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <Field label="Description" hint="Optional — product story or key benefits">
                      <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} placeholder="A richly formulated serum that..." className={`${inputCls} resize-none`} />
                    </Field>
                  </div>
                )}
                ...
                {step === 3 && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="rounded-3xl border border-[#1a1c1a]/8 bg-white/40 p-6 space-y-3">
                      <p className="font-manrope text-[10px] uppercase tracking-[0.3em] font-bold text-[#1a1c1a]/40">Update Summary</p>
                      <SummaryRow label="Name" value={name} />
                      <SummaryRow label="Type" value={type} />
                      <SummaryRow label="Price" value={formattedPrice} />
                      <SummaryRow label="Images" value={mediaItems.length > 0 ? `${mediaItems.length} added` : "—"} />
